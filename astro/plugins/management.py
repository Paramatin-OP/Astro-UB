# Re-written for astro by @its_xditya
...
@astro.on(admin_cmd(pattern="unbanall ?(.*)"))
@astro.on(sudo_cmd(pattern="unbanall ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@astro.on(admin_cmd(pattern="skick ?(.*)"))
@astro.on(sudo_cmd(pattern="skick ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    if event.is_private:
    ...
    async for i in borg.iter_participants(event.chat_id):
        ...
        if i.bot:
            ...
            n = n + 1
    if input_str:
        required_string = """astro has Kicked {} / {} users, out of which -
Deleted Accounts: {}
UserStatusEmpty: {}
UserStatusLastMonth: {}
UserStatusLastWeek: {}
