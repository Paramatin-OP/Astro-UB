...
@astro.on(admin_cmd(pattern="getqr"))
@astro.on(sudo_cmd(pattern="getqr", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
...
@astro.on(admin_cmd(pattern="makeqr ?(.*)"))
@astro.on(sudo_cmd(pattern="makeqr ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
